        ...
        <div class="bg-white dark:bg-gray-800 shadow-md sm:rounded-lg overflow-hidden w-full max-w-lg p-4">
            ...
            <div class="border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white p-2 pb-3">
                    Atur Status Hasil Layanan
                </h3>
            </div>

            <form action="{{route ('pic-ldi.hasil-layanan.store', ['id' => $permohonan->id]) }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 mt-5">
                    <!-- Kode Permohonan -->
                    <label for="kode_permohonan" class="block mb-2 text-sm font-medium text-gray-900">Kode Permohonan:</label>
                    <input type="text" class="text-sm bg-gray-200 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 cursor-not-allowed" name="kode_permohonan" id="kode_permohonan" value="{{ $permohonan->kode_permohonan }}" readonly>
                </div>
                <!-- Status -->
                <div class="grid grid-cols-1">
                    ...
                    <select id="status" name="status" class="text-sm border border-gray-200 rounded-lg block w-full p-2.5 transition-all" onchange="toggleKoreksi()">
                        @php
                            $status_sekarang = isset($permohonan->status) ? $permohonan->status : 'pending';
                        @endphp
                        <option value="pending" class="bg-yellow-200 text-yellow-900" {{ $status_sekarang == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="revisi" class="bg-red-200 text-red-900" {{ $status_sekarang == 'revisi' ? 'selected' : '' }}>Revisi</option>
                        <option value="disetujui" class="bg-green-200 text-green-900" {{ $status_sekarang == 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                    </select>
                </div>
                <!-- Koreksi -->
                <div id="koreksi-container" class="grid grid-cols-1 {{ $status_sekarang == 'revisi' ? '' : 'hidden' }}">
                    <label for="koreksi" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Koreksi <span class="text-redNew">*</span>
                    </label>
                    <textarea id="koreksi" name="koreksi" class="text-sm border border-gray-300 rounded-lg block w-full p-2.5"></textarea>
                </div>
